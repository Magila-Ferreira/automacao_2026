...
async function gerarGraficosGerenciais(dadosEscala) {
    ...
    const dadosConvertidos = Array.isArray(dadosEscala) ? dadosEscala : Object.values(dadosEscala).flat();
    ...
    while (tentativa < maximoTentativas) {
        try {
            for (const [escala, listaDeFatores] of Object.entries(escalasAgrupadas)) {
                ...
                const riscos = listaDeFatores.map((f) => Number(f.porcentagem_risco.toFixed(1)));

                const corBarra = riscos.map((valor) => (valor <= 40 ? '#080' : valor <= 80 ? '#cc0' : '#c00'));

                const urlGrafico = `https://quickchart.io/chart?width=${largura}&height=${altura}&c=${encodeURIComponent(
                    JSON.stringify({
                        type: 'horizontalBar',
                        data: {
                            labels: fatores,
            ...
            console.log(`✅ Gráficos GERENCIAIS gerados com sucesso!`);
