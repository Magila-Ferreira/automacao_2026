...
import { NOME_BANCO_CONTROLE, USUARIO_BD, gerenciadorDeConexoesBD } from '../config/configBanco.js';
...
const salvarDados = async (dados, nomeDoBanco, colunasDasRespostasExcel) => {
    ...
    try {
        ...
        for (const item of dados) {
            ...
            await db.query(inserir_identificacao, valores_identificacao);
            const id_identificacao = parseInt(item.id, 10); // Usa o id original do item

            if (id_identificacao) {
                // Insere as respostas associadas ao id_identificacao
                for (let i = 0; i < colunasDasRespostasExcel.length; i++) {
                    // Itera sobre as colunas respostas
...
async function limparBancosAntigos() {
    ...
    try {
		...
		console.error(`Erro ao consultar bancos antigos: ${error.message}`);
    } finally {
		await db.end();
    }   
}
...
const atualizarDadosGerenciaisSetor = async (dados, nomeDoBanco, instrucao_sql) => {
    ...
    try {
        for (const valores of dados) {
            ...
            if (typeof fator === 'number' && typeof area_setor === 'string' && typeof porcentagem_risco === 'number') {
                await db.query(instrucao_sql, valores);
...
const salvarRegistrosGerenciaisSetor = async (dadosTratadosSetor, nomeDoBanco) => {
    ...
    if (dadosBancoSetor.length === 0) {
        ...
        const atualizaveis = registrosDiferentesSetor
            .filter(
                ({ area_setor, fator, porcentagem_risco }) =>
                    typeof area_setor === 'string' && typeof fator === 'number' && typeof porcentagem_risco === 'number',
            )
